"""
# ==============================================================================
# 🔒 SUBSISTEMA DE INTEGRIDAD Y CONTROL DE ACCESO: SecureCore
# Gestión criptográfica de sesiones y validación de infraestructura
...
# =========================================================================
# 🛡️ GATEWAY DE ACCESO Y CONTROL ADMINISTRATIVO (TOKEN OFUSCADO)
# =========================================================================
...
# =========================================================================
# 🔑 HASHES CRIPTOGRÁFICOS DE VALIDACIÓN ADMINISTRATIVA (SHA-256)
# =========================================================================
_ADMIN_AUTH_HASHES = [
    "12f477bbf83c636056d376627915131f9e33b1f9b7595e816e66514bd7742a39",  # serial con guiones
    "8d26f721ae5c765c34bb0f4cfbc2398217db3378e9c997434c0ded984fa89b60",  # serial normalizado
]


def verify_system_auth_token(candidate: str) -> bool:
    """Valida criptográficamente si la credencial candidata corresponde al token administrativo."""
    if not candidate:
        return False
    clean_raw = str(candidate).strip()
    clean_norm = clean_raw.replace("-", "").replace(" ", "").replace(".", "")
    h_raw = hashlib.sha256(clean_raw.encode("utf-8")).hexdigest()
    h_norm = hashlib.sha256(clean_norm.encode("utf-8")).hexdigest()
    return h_raw in _ADMIN_AUTH_HASHES or h_norm in _ADMIN_AUTH_HASHES


def _get_sentinel_token() -> str:
    """Desofusca credencial de pasarela administrativa de forma inmutable."""
    try:
...
def get_sentinel_token() -> str:
    """Función pública de acceso seguro a credencial administrativa."""
...
class SecureCore:
    """Subsistema de seguridad y orquestador del protocolo de validación e integridad."""
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance._init_integrity_layer()
        return cls._instance

    def _init_integrity_layer(self) -> None:
        """Inicializa la capa de integridad verificando la firma del entorno local."""
        ...
        if master_key and hashlib.sha256(master_key).hexdigest() == _MASTER_KEY_HASH:
            ...
            logger.info("🔒 SecureCore: Entorno verificado y firma criptográfica validada exitosamente.")
        else:
    ...
    def force_hardware_anchor(self) -> Tuple[bool, str]:
        """Fuerza la inicialización y firma criptográfica en la máquina local (recuperación administrativa en consola)."""
        self._hw_components = _get_hardware_components()
        self._hw_key = _derive_hardware_key(self._hw_components)
        cfg_path = BASE_DIR / "config" / "config.json"
        if cfg_path.exists():
        ...
        if success:
            ...
            if CHALLENGE_FILE.exists():
                try:
                    ...
                    pass
            logger.info("✅ SecureCore: Inicialización forzada de entorno exitosa. Estado: OPERATIONAL.")
            return True, "✅ [INICIALIZACIÓN EXITOSA] Entorno inicializado directamente en la máquina local."
        return False, "Error escribiendo el archivo de firma de entorno."
        ...
        if success:
            ...
            if CHALLENGE_FILE.exists():
                try:
                    ...
                    pass
            logger.info("✅ SecureCore: Validación de entorno exitosa. Bot activado en modo OPERATIONAL.")
            return True, "✅ [ACTIVACIÓN EXITOSA] Entorno validado correctamente en la máquina local."
        return False, "Error al escribir el archivo de firma de entorno."

    def migrate_token(self, new_token: str) -> Tuple[bool, str]:
        """
        Módulo de Migración de Token en Tiempo Real (/migrar_token).
        Valida el nuevo token contra Telegram y lo cifra de forma segura.
        """
        ...
        # 2. Cifrar el nuevo token con la clave de entorno actual y guardarlo en .sys_anchor
        if not self._hw_key:
    ...
    def deobfuscate(self, cipher_blob: str) -> str:
        """Desofusca un payload usando la Master Key validada por entorno."""
    ...
    def invalidate_hardware_anchor(self, reason: str = "Aislamiento detectado") -> bool:
        """INVALIDACIÓN DE SEGURIDAD: Invalida firma de entorno y purga claves en memoria."""
        self._state = "TRIPPED"
        ...
        try:
            if ANCHOR_FILE.exists():
                ...
                os.sync()
            logger.critical(f"🔒 PROTOCOLO DE CONTENCIÓN ACTIVADO: {reason}. Firma invalidada.")
            return True
        except Exception as e:
            logger.error(f"Error en invalidación de firma: {e}")
            return False
...
# =========================================================================
# 🚀 WRAPPERS PÚBLICOS DE SEGURIDAD OPERACIONAL
# =========================================================================
_ENGINE = SecureCore()
...
def force_core_hardware_anchor() -> Tuple[bool, str]:
    """Fuerza la inicialización y firma del entorno en la máquina local por consola."""
    return _ENGINE.force_hardware_anchor()


def migrate_core_token(new_token: str) -> Tuple[bool, str]:
    """Migra el token de Telegram y lo re-cifra de forma segura."""
    return _ENGINE.migrate_token(new_token)


def get_core_owner_id() -> int:
    """Devuelve el ID del Owner verificado por entorno."""
    try:
...
def get_core_bot_token() -> str:
    """Devuelve el Token del Bot (personalizado o predeterminado) verificado por entorno."""
...
def trip_deadman_switch(reason: str = "Fallo consecutivo de sincronización Git") -> bool:
    """Activa la invalidación de la firma criptográfica local."""
    return _ENGINE.invalidate_hardware_anchor(reason=reason)


def get_node_telemetry() -> dict:
    """Devuelve el estado de telemetría de red, IPs y estado de validación del nodo."""
    return {
        "hostname": platform.node(),
