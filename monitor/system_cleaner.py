...
async def execute_clean_task(action: str) -> str:
    ...
    elif action == "clean_audit_artifacts":
        try:
            from monitor.network_analyzer import purge_old_audit_files
            count = purge_old_audit_files(max_days=30)
            return f"✅ <b>Auditoría optimizada:</b> {count} archivos depurados (.pcap residuales y reportes &gt;30d)."
        except Exception as e:
