...
def print_banner() -> None:
    ...
    print(f"{C_BOLD}{C_GREEN} 🛡️  VALIDACIÓN Y ACTIVACIÓN DE SERVICIOS (SHELL) {C_RESET}")
    print(f"{C_BOLD}{C_CYAN}=============================================================={C_RESET}")
...
{C_BOLD}DESCRIPCIÓN:{C_RESET}
  ...
  inicializar el entorno operativo y desbloquear los servicios
...
def show_status() -> None:
    ...
    print(f"⚓ {C_BOLD}Firma de Integridad:{C_RESET}  {'Presente (.sys_anchor)' if ANCHOR_FILE.exists() else 'No existe'}")
    ...
    if not args:
        ...
        if first in ("-h", "--help", "ayuda"):
            ...
            if ok:
                ...
                print(f"\n{C_BOLD}{C_GREEN}🎉 ¡Sistema inicializado y 100% OPERATIVO!{C_RESET}\n")
                return 0
            else:
                print(f"{C_RED}[ERROR] No se pudo inicializar el sistema: {msg}{C_RESET}")
                return 1
        else:
            serial_input = first
