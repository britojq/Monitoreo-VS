...
class AdminProxyController extends Controller
{
    ...
    {
        $proxies = MonitoredProxy::orderBy('id')->get();
    ...
    public function update(Request $request, MonitoredProxy $proxy): RedirectResponse
    {
        ...
        // LÓGICA DE PROTECCIÓN DE CREDENCIALES:
        if ($request->boolean('remove_auth')) {
            // Remoción explícita solicitada por el usuario
            $validated['auth_userpass'] = null;
        } elseif (empty(trim((string)$request->input('auth_userpass')))) {
            // Si el campo se dejó vacío o en blanco, CONSERVAR las credenciales existentes
            $validated['auth_userpass'] = $proxy->auth_userpass;
        } else {
            // Se ingresó una credencial
            $validated['auth_userpass'] = trim((string)$request->input('auth_userpass'));
        }
