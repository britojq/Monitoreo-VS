...
class AdminSslController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        AuditLog::create([
            ...
            'event' => 'created',
            'module' => 'ssl',
            'auditable_type' => SslCertificate::class,
            'auditable_id' => $cert->id,
            'entity_name' => 'Certificado SSL',
            'entity_label' => "{$cert->domain}:{$cert->port}",
            'description' => "Certificado SSL registrado para '{$cert->domain}:{$cert->port}' por " . ($user?->name ?? 'Usuario'),
            'new_values' => ['domain' => $cert->domain, 'port' => $cert->port],
    ...
    public function recheck(int $id): JsonResponse|RedirectResponse
    {
        ...
        AuditLog::create([
            ...
            'entity_name' => 'Certificado SSL',
            'entity_label' => "{$cert->domain}:{$cert->port}",
            'description' => "Certificado SSL para '{$cert->domain}' re-inspeccionado por " . ($user?->name ?? 'Usuario') . ". Estado: {$cert->last_check_status}",
            'new_values' => ['domain' => $cert->domain, 'status' => $cert->last_check_status],
            'ip_address' => request()->ip(),
            'user_agent' => request()->userAgent(),
        ]);
    ...
    public function recheckAll(): RedirectResponse
    {
        ...
        AuditLog::create([
            ...
            'auditable_type' => SslCertificate::class,
            'auditable_id' => null,
            'entity_name' => 'Certificados SSL',
            'entity_label' => 'Inspección Masiva',
            'description' => "Todos los certificados SSL activos fueron re-inspeccionados por " . ($user?->name ?? 'Usuario'),
            'new_values' => ['action' => 'recheck_all'],
            'ip_address' => request()->ip(),
            'user_agent' => request()->userAgent(),
        ]);
    ...
    public function destroy(int $id): RedirectResponse
    {
        ...
        AuditLog::create([
            ...
            'entity_name' => 'Certificado SSL',
            'entity_label' => $domain,
            'description' => "Certificado SSL para '{$domain}' eliminado del monitoreo por " . ($user?->name ?? 'Usuario'),
