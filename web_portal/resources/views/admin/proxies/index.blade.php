    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 mb-1.5">
                ...
                <span class="text-xs font-mono text-obsidian-muted">Proxies Corporativos</span>
            </div>
                ...
                <tbody class="divide-y divide-obsidian-border/60">
                    @foreach($proxies as $p)
                        <tr class="hover:bg-obsidian-panel/40 transition">
                            <td class="px-6 py-4">
                                <span class="px-2 py-0.5 rounded text-[11px] font-mono font-bold bg-obsidian-panel text-obsidian-cyan border border-obsidian-border">
                                    #{{ $p->id }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right space-x-1.5">
                                ...
                                @if(auth()->user()->isAdmin())
                                    ...
                                    @else
                                        <button onclick="openEditProxyModal({{ json_encode($p->makeVisible('auth_userpass')) }})" class="px-2.5 py-1.5 rounded-lg bg-obsidian-panel border border-obsidian-border text-obsidian-cyan hover:bg-obsidian-cyan hover:text-black font-mono text-xs transition flex items-center gap-1 inline-flex" title="Editar Parámetros">
                                            <span class="material-symbols-outlined text-sm">edit</span>
                                            <span>Modificar</span>
                                        </button>
                                        <form action="{{ route('admin.proxies.destroy', $p->id) }}" method="POST" class="inline" onsubmit="return confirm('¿Está seguro de eliminar el proxy [{{ addslashes($p->name) }}]?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="px-2 py-1.5 rounded-lg bg-obsidian-panel border border-red-500/40 text-red-400 hover:bg-red-500 hover:text-white font-mono text-xs transition inline-flex items-center gap-1 shadow-sm" title="Eliminar Proxy">
    ...
    <div class="glass-panel max-w-md w-full rounded-2xl p-6 border border-obsidian-border shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto custom-scroll">
        ...
        <form id="form-edit-proxy" method="POST" class="space-y-4 font-mono text-xs">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="block text-obsidian-muted mb-1">Identificador</label>
                    <span id="edit_p_id_badge" class="px-3 py-2 rounded-lg bg-obsidian-panel border border-obsidian-border font-bold text-obsidian-cyan text-xs w-full block text-center">#--</span>
                    <input type="hidden" name="letter" id="edit_p_letter" required maxlength="5"/>
                </div>
                <div>
            ...
            <div>
                <div class="flex items-center justify-between mb-1">
                    <label class="block text-obsidian-muted">Credenciales (usuario:clave)</label>
                    <span class="text-[10px] text-obsidian-cyan/90 font-mono">Dejar vacío conserva la clave</span>
                </div>
                <div class="relative">
                    <input type="text" name="auth_userpass" id="edit_p_auth" class="w-full bg-obsidian-panel border border-obsidian-border rounded-lg p-2 pr-10 text-white font-mono text-xs placeholder:text-obsidian-muted/40" placeholder="usuario:clave"/>
                    <button type="button" onclick="toggleAuthVisibility()" id="btn-toggle-auth" class="absolute right-2 top-1/2 -translate-y-1/2 text-obsidian-muted hover:text-white p-1 rounded transition" title="Alternar visibilidad">
                        <span class="material-symbols-outlined text-sm" id="icon-toggle-auth">visibility_off</span>
                    </button>
                </div>
                <div class="mt-1.5 flex items-center gap-2">
                    <input type="checkbox" name="remove_auth" id="edit_p_remove_auth" value="1" class="rounded bg-obsidian-panel border-obsidian-border text-red-500 focus:ring-0 cursor-pointer" onchange="handleRemoveAuthToggle(this)"/>
                    <label for="edit_p_remove_auth" class="text-[10.5px] text-red-400/80 hover:text-red-400 cursor-pointer select-none">Remover credenciales (dejar proxy sin autenticación)</label>
                </div>
            </div>
    ...
    <div class="glass-panel max-w-4xl w-full rounded-2xl border border-obsidian-border/90 shadow-2xl space-y-4 max-h-[95vh] overflow-y-auto custom-scroll p-5 sm:p-6 bg-[#07172b]/95">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-obsidian-border/80 pb-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="flex items-center gap-2">
                        <span id="hist-proxy-letter" class="text-xs font-mono font-bold text-amber-400 bg-obsidian-panel px-2 py-0.5 rounded border border-obsidian-border">#--</span>
...
<script>
    ...
    function toggleAuthVisibility() {
        const input = document.getElementById('edit_p_auth');
        const icon = document.getElementById('icon-toggle-auth');
    ...
    function handleRemoveAuthToggle(cb) {
        const input = document.getElementById('edit_p_auth');
        if (cb.checked) {
            input.value = '';
            input.disabled = true;
            input.placeholder = 'Sin autenticación (credenciales removidas)';
        } else {
            input.disabled = false;
            input.placeholder = 'usuario:clave';
            if (currentEditingProxy && currentEditingProxy.auth_userpass) {
                input.value = currentEditingProxy.auth_userpass;
            }
        }
    }

    function openEditProxyModal(p) {
        currentEditingProxy = p;
        document.getElementById('form-edit-proxy').action = `/admin/proxies/${p.id}`;
        document.getElementById('edit_p_id_badge').innerText = `#${p.id}`;
        ...
        const authInput = document.getElementById('edit_p_auth');
        const removeAuthCb = document.getElementById('edit_p_remove_auth');
        if (removeAuthCb) removeAuthCb.checked = false;
        if (authInput) {
            authInput.disabled = false;
            authInput.value = p.auth_userpass || '';
            authInput.type = 'text';
        }
        const toggleIcon = document.getElementById('icon-toggle-auth');
        if (toggleIcon) toggleIcon.innerText = 'visibility_off';
    ...
    async function openProxyHistoryModal(proxyId, name, letter, ipPort) {
        ...
        document.getElementById('hist-proxy-letter').innerText = `#${proxyId}`;
