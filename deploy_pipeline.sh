...
sync_web_portal() {
    ...
    if [ -f "$WEB_DIR/database/seeders/SnmpOidsSeeder.php" ]; then
        sudo php "$WEB_DIR/artisan" db:seed --class=SnmpOidsSeeder --force >/dev/null 2>&1 || true
    fi
    ...
    # Ejecutar Hook de post-actualización mayor y cambios a nivel de SO
    if [ -f "$BASE_DIR/post_update.py" ]; then
        log "⚙️ Ejecutando hook de cambios mayores (post_update.py)..."
        sudo "$PYTHON_BIN" "$BASE_DIR/post_update.py" 2>&1 | tee -a "$LOG_FILE" || true
    fi
}
