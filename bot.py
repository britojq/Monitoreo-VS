...
def log_security_audit_event(
    ...
    """Registra un evento de seguridad tanto en la base de datos MariaDB (audit_logs) como en el archivo local."""
    now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    clean_username = (username or "").lstrip("@")
    user_label = f"{full_name} (@{clean_username})" if clean_username else (full_name or str(user_id))

    # 1. Registro en Base de Datos MariaDB (audit_logs)
    try:
        ...
        with conn.cursor() as cur:
            sql = """
                ...
                (user_id, user_name, user_email, user_role, event, module, entity_name, entity_label, description, new_values, ip_address, created_at, updated_at)
            ...
            cur.execute(sql, (
                None,
                user_label[:255],
                f"{user_id}@telegram.local",
                "unauthorized",
                (event or "unauthorized_access")[:255],
                "bot",
                "Telegram Bot",
                command[:255] if command else "Telegram Command",
                description or f"Intento no autorizado de {user_label} en {chat_title} ({chat_id}): {command}",
                json.dumps(payload, ensure_ascii=False),
                "127.0.0.1"
            ))
            conn.commit()
    except Exception as e_db:
        logger.warning(f"Aviso guardando auditoría en BD: {e_db}")
    finally:
        try:
            conn.close()
        except Exception:
    ...
    # 2. Respaldo en archivo local (intentos_acceso.log)
    if CONFIG.get("log_unauthorized_to_file", True):
        audit_path = get_audit_log_path()
        log_line = (
            f"[{now_str}] {event.upper()} | ID: {user_id} | Username: {username} | "
            f"Nombre: {full_name} | Chat: {chat_title} (ID: {chat_id}) | "
            f"Comando/Mensaje: {command}\n"
        )
        try:
            with open(audit_path, "a", encoding="utf-8") as f:
                f.write(log_line)
        except Exception as e_f:
            logger.error(f"Error escribiendo en {audit_path}: {e_f}")
...
async def check_authorization(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """
    Verifica la autorización del usuario, chat y estado de activación del sistema.
    Si el sistema está en estado FIRST_BOOT_PENDING o PENDING_VALIDATION:
    - Permite al Owner enviar el Serial de Activación para desbloquear el sistema.
    ...
    # 0. Verificación de Estado de Activación e Integridad del Sistema
        ...
        if user and user.id == IMMUTABLE_OWNER_ID and serial_candidate:
            ...
            if ok:
                await safe_reply_html(
                    update.message,
                    f"{msg}\n\n"
                    "🎉 <b>¡Bienvenido!</b> El sistema ha completado la validación de integridad y se encuentra ahora 100% <b>OPERACIONAL</b>."
                )
                return True
            else:
                await safe_reply_html(
    ...
    # 1. Registrar en BD y archivo de auditoría
    log_security_audit_event(
        ...
        chat_title=chat_title,
        command=msg_text,
        description=f"Intento no autorizado en chat {chat_title} (ID: {chat_id}, Tipo: {chat_type}): {msg_text}",
        extra_data={"lang": lang, "chat_type": chat_type}
    )
...
async def cmd_analisis_red(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Parsear argumentos opcionales (ej. /red, /red vivo, /red vivo 60, /red debug)
    duration = 60
    force_debug = False
    is_live = False
    ...
    if context.args:
        for arg in context.args:
            ...
            if arg_clean in ("vivo", "live", "captura", "raw", "scan"):
                is_live = True
    ...
    # 1. MODO NET RADAR RÁPIDO (< 1s): Telemetría consolidada desde Base de Datos
    if not is_live:
        try:
            from monitor.net_radar_engine import generate_markdown_report_from_db
            summary_text, md_file, _ = generate_markdown_report_from_db()
            ...
            if md_file and md_file.exists():
                with open(md_file, "rb") as f:
                    await context.bot.send_document(
                        ...
                        caption="📄 Reporte estructurado NET Radar (Tráfico y Actualizaciones)"
                    )
            ...
            if is_allowed_group and not is_owner and owner_id:
                try:
                    ...
                    await context.bot.send_message(
                        chat_id=owner_id,
                        text=f"📋 <b>Copia de Auditoría: NET Radar ejecutado en {html.escape(group_title)}</b>\n\n" + summary_text,
                        parse_mode='HTML'
                    )
                    if md_file and md_file.exists():
                        with open(md_file, "rb") as f:
                            await context.bot.send_document(
                                ...
                                document=f,
                                filename=md_file.name,
                                caption=f"📄 Reporte NET Radar ({group_title})"
                            )
                except Exception as e:
                    logger.error(f"Error enviando copia de auditoría al Owner: {e}")
            return
        except Exception as e_fast:
            logger.error(f"Error generando reporte rápido NET Radar desde BD: {e_fast}", exc_info=True)
            # Continúa a modo en vivo si la consulta falla

    # 2. MODO EN VIVO ON-DEMAND (/red vivo): Captura activa con tcpdump y tshark
    wait_msg = await update.message.reply_text(
...
# =========================================================================
# 🔄 MÓDULO DE MIGRACIÓN DINÁMICA DE TOKEN (/migrar_token)
# =========================================================================
...
async def cmd_migrar_token_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await safe_reply_html(
        ...
        "Este proceso re-cifrará el nuevo token con los estándares de seguridad locales sin exponerlo en texto plano.\n\n"
...
async def handle_new_token_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Procesa, valida con Telegram y re-cifra el nuevo token de forma segura."""
...
async def cmd_activar_manual(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Permite al Owner activar manualmente la verificación de integridad usando /activar <SERIAL> (Exclusivo en privado)."""
    if not await require_private_chat(update, context):
        return
...
async def cmd_lockdown(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Activa el modo LOCKDOWN de contención perimetral inmediata (Exclusivo Administrador)."""
    user_id = update.effective_user.id if update.effective_user else 0
    owner_id = int(CONFIG.get("owner_id", 38914901))
    allowed_admins = set(CONFIG.get("allowed_user_ids", [])) | {IMMUTABLE_OWNER_ID, owner_id}
    if user_id not in allowed_admins:
        if update.message:
            await update.message.reply_text("⛔ Acceso denegado: Comando exclusivo del Administrador.")
        return

    from monitor.terminal_shield import execute_lockdown
    execute_lockdown(target_user=update.effective_user.username or "admin")
    hostname = socket.gethostname()
    now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    msg = (
        "🚨🚨 <b>MODO LOCKDOWN ACTIVADO</b> 🚨🚨\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🖥️ <b>Servidor:</b> <code>{html.escape(hostname)}</code>\n"
        f"⏰ <b>Hora:</b> <code>{now_str}</code>\n\n"
        "🛡️ <b>Medidas de contención aplicadas de inmediato:</b>\n"
        " • Todas las consolas y emuladores (Konsole, xterm) han sido terminados.\n"
        " • Pantalla y sesiones gráficas bloqueadas.\n"
        " • Servidor VNC detenido temporalmente.\n"
        " • Ejecución de comandos interactivos y sudo restringidos.\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "<i>Para restaurar la operación normal use el botón inferior o /unlock</i>"
    )
    keyboard = [
        [InlineKeyboardButton("🔓 DESACTIVAR LOCKDOWN", callback_data=f"shield_unlock:{hostname}")]
    ]
    if update.message:
        await safe_reply_html(update.message, msg, reply_markup=InlineKeyboardMarkup(keyboard))
...
async def cmd_unlock(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Desactiva el modo LOCKDOWN y restaura los servicios interactivos."""
    user_id = update.effective_user.id if update.effective_user else 0
    owner_id = int(CONFIG.get("owner_id", 38914901))
    allowed_admins = set(CONFIG.get("allowed_user_ids", [])) | {IMMUTABLE_OWNER_ID, owner_id}
    if user_id not in allowed_admins:
    ...
    from monitor.terminal_shield import execute_unlock
    execute_unlock()
    hostname = socket.gethostname()

    msg = (
        "✅ <b>MODO LOCKDOWN DESACTIVADO</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🖥️ <b>Servidor:</b> <code>{html.escape(hostname)}</code>\n\n"
        "Operación normal restablecida:\n"
        " • Servicio VNC reactivado.\n"
        " • Acceso administrativo y terminales desbloqueados.\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━"
    )
    if update.message:
        await safe_reply_html(update.message, msg)
...
async def cmd_autorizar_consola(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Habilita una ventana de tiempo autorizada para que el Administrador use terminales sin ser alertado."""
    user_id = update.effective_user.id if update.effective_user else 0
    owner_id = int(CONFIG.get("owner_id", 38914901))
    allowed_admins = set(CONFIG.get("allowed_user_ids", [])) | {IMMUTABLE_OWNER_ID, owner_id}
    if user_id not in allowed_admins:
    ...
    from monitor.terminal_shield import execute_auth
    res = execute_auth(minutes)
    hostname = socket.gethostname()

    msg = (
        "🛡️ <b>VENTANA DE MANTENIMIENTO AUTORIZADA</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🖥️ <b>Servidor:</b> <code>{html.escape(hostname)}</code>\n"
        f"⏱️ <b>Duración:</b> <code>{minutes} minutos</code>\n"
        f"⏰ <b>Válido hasta:</b> <code>{res.get('expiry', 'N/A')}</code>\n\n"
        "El Administrador puede abrir Konsole, terminales interactivas y ejecutar comandos sudo "
        "sin que se disparen alertas ni bloqueos preventivos.\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━"
    )
    if update.message:
        await safe_reply_html(update.message, msg)
...
async def handle_shield_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Maneja las acciones interactivas de mitigación del Sentinel Shield (Lockdown, Kill, Auth)."""
    query = update.callback_query
    if not query or not query.data:
        return
    ...
    try:
        from monitor.terminal_shield import execute_kill, execute_lockdown, execute_unlock, execute_auth

        data_parts = query.data.split(":")
        action_prefix = data_parts[0]
        target_server = data_parts[1] if len(data_parts) > 1 else ""
        target_pid = int(data_parts[2]) if len(data_parts) > 2 and data_parts[2].isdigit() else 0
        target_ppid = int(data_parts[3]) if len(data_parts) > 3 and data_parts[3].isdigit() else 0

        hostname = socket.gethostname()

        if action_prefix == "shield_kill":
            await query.answer("🚫 Neutralizando terminal...")
            res = execute_kill(target_pid, target_ppid)
            killed_str = ", ".join(res.get("killed", [])) or f"PID {target_pid}"
            infractions = res.get("infractions", 1)
            reboot_triggered = res.get("reboot_triggered", False)

            if reboot_triggered:
                text = (
                    "🛑 <b>REINCIDENCIA MÁXIMA ALCANZADA (3/3)</b>\n"
                    "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"🖥️ <b>Servidor:</b> <code>{html.escape(target_server or hostname)}</code>\n"
                    f"⚡ <b>Procesos Eliminados:</b> <code>{html.escape(killed_str)}</code>\n"
                    f"⚠️ <b>Infracciones Acumuladas:</b> <code>3 de 3</code>\n"
                    f"👤 <b>Acción ejecutada por:</b> Administrador (@{html.escape(query.from_user.username or 'admin')})\n"
                    "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                    "🚨 <i>Se ha activado el protocolo de reinicio por reincidencia. El equipo se reiniciará en 10 segundos tras la advertencia visual en pantalla.</i>"
                )
            else:
                text = (
                    "🚫 <b>TERMINAL NEUTRALIZADA EXITOSAMENTE</b>\n"
                    "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"🖥️ <b>Servidor:</b> <code>{html.escape(target_server or hostname)}</code>\n"
                    f"⚡ <b>Procesos Eliminados:</b> <code>{html.escape(killed_str)}</code>\n"
                    f"⚠️ <b>Infracción Registrada:</b> <code>{infractions} de 3</code>\n"
                    "📺 <b>Acción en Pantalla:</b> Advertencia de 10s desplegada al operador.\n"
                    f"👤 <b>Acción ejecutada por:</b> Administrador (@{html.escape(query.from_user.username or 'admin')})\n"
                    "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            ...
            try:
                ...
                logger.warning(f"No se pudo editar mensaje de callback: {e_edit}")

        elif action_prefix == "shield_lockdown":
            await query.answer("🔒 ACTIVANDO REINICIO DE SEGURIDAD...", show_alert=True)
            execute_lockdown(target_pid, target_ppid)
            text = (
                "🚨🚨 <b>REINICIO DE CONTENCIÓN ACTIVADO</b> 🚨🚨\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                f"🖥️ <b>Servidor:</b> <code>{html.escape(target_server or hostname)}</code>\n"
                f"⏰ <b>Hora:</b> <code>{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</code>\n\n"
                "🛡️ <b>Medidas de contención activadas:</b>\n"
                f" • Terminal interactiva terminada (PID: {target_pid}).\n"
                " • Todas las instancias de emuladores cerradas.\n"
                " • Diálogo visual de seguridad desplegado en pantalla con cuenta regresiva de 10s.\n"
                " • Reinicio automático del equipo en ejecución para neutralizar la intrusión.\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                "<i>El equipo se reiniciará automáticamente al concluir los 10 segundos.</i>"
            )
            try:
                await query.edit_message_text(text, parse_mode='HTML', reply_markup=None)
            except Exception as e_edit:
        ...
        elif action_prefix == "shield_unlock":
            await query.answer("🔓 Desactivando estado de emergencia...")
            execute_unlock()
            text = (
                "✅ <b>SISTEMA RESTAURADO A LA NORMALIDAD</b>\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                f"🖥️ <b>Servidor:</b> <code>{html.escape(target_server or hostname)}</code>\n\n"
                "El estado de emergencia ha sido levantado:\n"
                " • Infracciones acumuladas reseteadas a 0.\n"
                " • Permisos interactivos reactivados.\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━━━"
            )
            try:
                await query.edit_message_text(text, parse_mode='HTML', reply_markup=None)
        ...
        elif action_prefix == "shield_auth":
            await query.answer("✅ Sesión autorizada por 30 minutos.")
            res = execute_auth(30)
            text = (
                "✅ <b>SESIÓN AUTORIZADA POR EL ADMINISTRADOR</b>\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                f"🖥️ <b>Servidor:</b> <code>{html.escape(target_server or hostname)}</code>\n"
                f"🆔 <b>Terminal:</b> PID <code>{target_pid}</code>\n"
                f"⏱️ <b>Ventana de Trabajo:</b> <code>30 minutos</code> (hasta {res.get('expiry', 'N/A')})\n"
                "🛡️ <b>Infracciones:</b> Contador reseteado a 0.\n\n"
                "<i>Puede continuar utilizando la terminal sin interrupciones ni advertencias.</i>\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━━━"
            )
            try:
                await query.edit_message_text(text, parse_mode='HTML', reply_markup=None)
        ...
        logger.error(f"Error procesando shield callback: {e_cb}", exc_info=True)
        try:
            await query.answer(f"⚠️ Error ejecutando acción: {e_cb}", show_alert=True)
        except Exception:
            pass
...
async def handle_emergency_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "close":
        ...
        prompt_text = (
            ...
            "<i>Las credenciales protegidas y la identidad del servidor se mantendrán intactas.</i>"
        )
        keyboard = [
            [
...
def main() -> None:
    ...
    # Conversación exclusiva para que el Owner migre el Token del Bot
    ...
    # Comandos de Contención y Escudo de Terminal (Sentinel Shield) (Exclusivo Owner)
    application.add_handler(CommandHandler(["lockdown", "bloqueo_total"], cmd_lockdown))
    application.add_handler(CommandHandler(["unlock", "desbloqueo_total"], cmd_unlock))
    application.add_handler(CommandHandler(["autorizar_consola", "consola", "auth_terminal"], cmd_autorizar_consola))

    # Callback query handler para Sentinel Terminal Shield (Lockdown, Kill, Auth, Unlock)
    application.add_handler(CallbackQueryHandler(handle_shield_callback, pattern=r"^shield_"))
