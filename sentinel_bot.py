...
def trigger_service_switchover() -> None:
    """Habilita e inicia tg-admin-bot y desactiva sentinel_bot de manera desatendida y segura."""
    def _switch():
        time.sleep(1.5)
        logger.info("🔄 Ejecutando transición de servicios: habilitando tg-admin-bot y desactivando sentinel_bot...")
        cmd_prefix = ["sudo"] if os.geteuid() != 0 else []
        try:
            subprocess.run(cmd_prefix + ["systemctl", "enable", "--now", "tg-admin-bot.service"], check=False)
        except Exception as e:
            logger.error(f"Error habilitando tg-admin-bot: {e}")
        try:
            subprocess.run(cmd_prefix + ["systemctl", "disable", "--now", "sentinel_bot.service"], check=False)
        except Exception as e:
            logger.error(f"Error desactivando sentinel_bot: {e}")
...
async def on_sentinel_startup(app: Application) -> None:
    """Notifica al Owner al iniciar el Bot Centinela si el nodo requiere Onboarding."""
    try:
        telem = get_node_telemetry()
        is_op = is_core_operational()
        state = telem.get("state", "UNKNOWN")
        serial = telem.get("active_serial")
        if not serial and CHALLENGE_FILE.exists():
            try:
                c_data = json.loads(CHALLENGE_FILE.read_text(encoding="utf-8"))
                serial = c_data.get("serial")
            except Exception:
                pass

        hostname = telem.get("hostname", platform.node())
        ips = ", ".join(telem.get("ips", []))

        if not is_op:
            text = (
                "🛡️ <b>[ONBOARDING CENTINELA: NUEVA INSTALACIÓN DETECTADA]</b>\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                f"🖥️ <b>Servidor:</b> <code>{hostname}</code>\n"
                f"🌐 <b>IPs Detectadas:</b> <code>{ips}</code>\n"
                f"🔒 <b>Estado DRM:</b> <code>{state}</code>\n"
            )
            if serial:
                text += (
                    f"\n🔑 <b>Serial de Activación:</b>\n<code>{serial}</code>\n\n"
                    "<b>Para autorizar esta instalación y activar el Bot Principal con su propio Token:</b>\n"
                    f"1️⃣ Envíe: <code>/activar {serial} &lt;TOKEN_BOT&gt;</code>\n"
                    "2️⃣ O presione el botón inferior para iniciar el asistente guiado."
                )
                keyboard = [
                    [InlineKeyboardButton("🚀 Activar y Asignar Token", callback_data=f"sentinel:onboard:{serial}")],
                    [InlineKeyboardButton("📊 Ver Telemetría", callback_data="sentinel:menu:status")]
                ]
            else:
                text += "\n<i>Esperando inicialización de desafío de hardware...</i>"
                keyboard = [
                    [InlineKeyboardButton("📊 Ver Telemetría", callback_data="sentinel:menu:status")]
                ]
            ...
            logger.info("📢 Notificación de Onboarding enviada al Owner exitosamente.")
    except Exception as e:
        logger.error(f"Error enviando notificación de startup en Sentinel Bot: {e}")
...
async def cmd_activar(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Comando de activación manual de hardware y asignación opcional de token."""
    if not await require_owner(update):
        return

    if not context.args or len(context.args) == 0:
        await update.message.reply_text(
            "ℹ️ <b>Uso del comando:</b> <code>/activar &lt;SERIAL&gt; [NUEVO_TOKEN_BOT]</code>\n\n"
            "<i>Ejemplo:</i>\n"
            "<code>/activar AUTH-A1B2-C3D4-E5F6-7890 123456789:ABCdef...</code>",
            parse_mode="HTML"
        )
        return

    serial = context.args[0].strip()
    token_candidate = context.args[1].strip() if len(context.args) > 1 else None

    bot_username = None
    if token_candidate:
        if ":" not in token_candidate or len(token_candidate) < 30:
            await update.message.reply_text(
                "❌ <b>Formato de Token Inválido:</b> Debe contener ':' y la longitud habitual proporcionada por BotFather.",
                parse_mode="HTML"
            )
            return

        status_validating = await update.message.reply_text("🔄 Validando token con Telegram...")
        try:
            async with httpx.AsyncClient(timeout=8.0) as client:
                r = await client.get(f"https://api.telegram.org/bot{token_candidate}/getMe")
                if r.status_code != 200 or not r.json().get("ok"):
                    await status_validating.edit_text(
                        f"❌ <b>Token Rechazado por Telegram (HTTP {r.status_code}):</b> Verifique el token de BotFather.",
                        parse_mode="HTML"
                    )
                    return
                bot_username = r.json().get("result", {}).get("username", "Desconocido")
                await status_validating.delete()
        except Exception as e:
            await status_validating.edit_text(f"❌ Error conectando con Telegram: {e}", parse_mode="HTML")
            return

    ok, msg = activate_hardware_first_boot(serial, custom_token=token_candidate)
    if ok:
        response_text = (
            f"🎉 <b>{msg}</b>\n\n"
            "✅ <b>Hardware anclado exitosamente en este servidor.</b>\n"
        )
        if bot_username:
            response_text += f"🤖 <b>Bot Principal Asignado:</b> <code>@{bot_username}</code>\n"

        response_text += (
            "\n🚀 <b>Transición de Servicios en Proceso:</b>\n"
            "• Habilitando y arrancando <code>tg-admin-bot.service</code>...\n"
            "• Desactivando <code>sentinel_bot.service</code> (Onboarding completado).\n\n"
            "<i>A partir de este momento, interactúe directamente con su Bot Principal.</i>"
        )
        await update.message.reply_text(response_text, parse_mode="HTML")
        trigger_service_switchover()
    else:
...
async def start_migration_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Inicia el flujo conversacional interactivo de migración en caliente o aprovisionamiento inicial."""
    if not await require_owner(update):
        return ConversationHandler.END

    onboard_serial = None
    if update.callback_query and update.callback_query.data and "onboard:" in update.callback_query.data:
        onboard_serial = update.callback_query.data.split("onboard:")[-1].strip()
        context.user_data["onboard_serial"] = onboard_serial

    telem = get_node_telemetry()
    is_op = is_core_operational()

    if not is_op:
        title = "🚀 <b>ASISTENTE DE APROVISIONAMIENTO Y ACTIVACIÓN INICIAL</b>\n"
        step_desc = "1️⃣ Ve a @BotFather y copia el <b>Token de Telegram</b> para este nuevo servidor.\n"
    else:
        title = "🔄 <b>ASISTENTE DE MIGRACIÓN EN CALIENTE (ZERO-DOWNTIME)</b>\n"
        step_desc = "1️⃣ Ve a @BotFather y copia el <b>Nuevo Token de Telegram</b> que se le asignará al servidor.\n"
...
async def handle_migration_token_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Si vino por onboarding con serial, activar hardware
    onboard_serial = context.user_data.get("onboard_serial")
    if onboard_serial:
        activate_hardware_first_boot(onboard_serial, custom_token=token_candidate)

    # Aplicar migración de token
    ok, msg = migrate_core_token(token_candidate)
    if ok:
        await status_msg.edit_text(
            f"🎉 <b>APROVISIONAMIENTO Y MIGRACIÓN COMPLETADOS EXITOSAMENTE</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"✅ <b>Nuevo Token Asignado:</b> <code>@{bot_username}</code>\n"
...
def main() -> None:
    ...
    migration_handler = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(start_migration_conversation, pattern=r"^sentinel:(menu:migrar|onboard:.*)$")
        ],
        states={
