...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    help_lines.append("/reset_ia - Reinicia la conversación con el asistente Ollama")
...
async def handle_dynamic_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not cmd_config:
        await update.message.reply_text(
            MESSAGES.get("unknown_command", "Comando no configurado."),
            parse_mode='HTML'
        )
        return
...
async def reset_chat(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Si el bot es público, puedes permitir reset a todos.
    # Por ahora, si no está en modo público, exige autorización.
